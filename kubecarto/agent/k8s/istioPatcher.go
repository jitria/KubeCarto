...
type meshConfig struct {
	...
	ExtraFields map[string]interface{} `yaml:",inline"` // all extra fields that Agent will not touch
}
...
func PatchIstioConfigMap() bool {
	...
	meshCfg.DefaultConfig.EnvoyAccessLogService.Address = "kubecarto-agent.kubecarto.svc.cluster.local:4317"
	meshCfg.DefaultConfig.EnvoyMetricsService.Address = "kubecarto-agent.kubecarto.svc.cluster.local:4317"
	...
	if patched, _ := isEnvoyOtelAlPatched(meshCfg); !patched {
		sfOtelAl := struct {
			...
			EnvoyOtelAls: struct {
				...
				Port:    "4317",
				Service: "kubecarto-agent.kubecarto.svc.cluster.local",
			},
			Name: "kubecarto-agent",
		}
	...
	if patched, _ := isEnvoyALProviderPatched(meshCfg); !patched {
		meshCfg.DefaultProviders.AccessLogs = append(meshCfg.DefaultProviders.AccessLogs, "kubecarto-agent")
	}
	...
	if err != nil {
		log.Printf("[PatchIstioConfigMap] Unable to marshal Istio ConfigMap: %v", err)
		return false
	}
...
func UnpatchIstioConfigMap() bool {
	...
	// @todo check if this shall actually be overwritten by Agent
...
func isEnvoyOtelAlPatched(meshCfg meshConfig) (bool, int) {
	for idx, envoyOtelAl := range meshCfg.ExtensionProviders {
		if envoyOtelAl.Name == "kubecarto-agent" &&
			envoyOtelAl.EnvoyOtelAls.Port == "4317" &&
			envoyOtelAl.EnvoyOtelAls.Service == "kubecarto-agent.kubecarto.svc.cluster.local" {
			return true, idx
		}
	}
...
func isEnvoyALProviderPatched(meshCfg meshConfig) (bool, int) {
	for idx, accessLogProvider := range meshCfg.DefaultProviders.AccessLogs {
		if accessLogProvider == "kubecarto-agent" {
			return true, idx
		}
	}
	return false, -1
...
func isIstioAlreadyPatched(meshCfg meshConfig) bool {
	if meshCfg.DefaultConfig.EnvoyAccessLogService.Address != "kubecarto-agent.kubecarto.svc.cluster.local:4317" ||
		meshCfg.DefaultConfig.EnvoyMetricsService.Address != "kubecarto-agent.kubecarto.svc.cluster.local:4317" {
		return false
	}
